...
fn recursive_tail_call() {
    ...
    eval!("(def! sum2 (fn* (n acc) (if (= n 0) acc (sum2 (- n 1) (+ n acc)))))", lock.deref_mut());
...
fn mutually_recursive_tail_call() {
    let env = testing_env();
    let mut lock = env.write();
    eval!("(def! foo (fn* (n) (if (= n 0) 0 (bar (- n 1)))))", lock.deref_mut());
    eval!("(def! bar (fn* (n) (if (= n 0) 0 (foo (- n 1)))))", lock.deref_mut());
